...
window.Topbar = (function() {
  ...
  function init(opts) {
    ...
    // Mark + product name as a single lockup. Mark is inline SVG generated by
    // window.mark() from brand-logos.js (canonical source). Two theme variants
    // in DOM, CSS hides one based on data-theme. Product name continues from
    // the mark P when it starts with "Penso" (mark + "ensoCodex") and falls
    // back to full name otherwise (ClassForge, TypeDrill, etc).
    ...
    if (displayName.indexOf('Penso') === 0) {
      // Mark P is the leading letter; show the rest in lowercase continuation.
      continuation = displayName.slice(1);
    } else {
      // Non-Penso product (ClassForge, TypeDrill); mark stays as brand anchor,
      // full product name beside it.
      continuation = displayName;
    }

    var markLight = document.createElement('span');
    markLight.className = 'bs-topbar-logo-light bs-topbar-mark';
    markLight.setAttribute('aria-hidden', 'true');
    if (window.mark && window.stdColors) {
      markLight.innerHTML = window.mark(window.stdColors('white'));
    }
    brand.appendChild(markLight);

    var markDark = document.createElement('span');
    markDark.className = 'bs-topbar-logo-dark bs-topbar-mark';
    markDark.setAttribute('aria-hidden', 'true');
    if (window.mark && window.stdColors) {
      markDark.innerHTML = window.mark(window.stdColors('navy'));
    }
    brand.appendChild(markDark);

    var name = document.createElement('span');
    name.className = 'bs-topbar-name';
    name.textContent = continuation;
