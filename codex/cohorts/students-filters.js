...
// Pure filter logic for the Alunos roster (track-28a2). DOM-free so it is unit-testable and
...
export function hasStatus(s, st) { return (s.turmas || []).some((x) => x.access_status === st); }
...
export function filterOptions(students) {
  ...
  const clients = Array.from(new Set(
    list.flatMap((s) => (s.turmas || []).map((x) => x.client_slug).filter(Boolean))
  )).sort();
