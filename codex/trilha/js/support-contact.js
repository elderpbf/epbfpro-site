...
// registro, bloqueado, erro screens, plus a footer strip on every page): a single
// source for the phone/e-mail + the rendered affordance, so students who get stuck
// (no code, blocked, error) always have a direct channel instead of borrowing
// Élder's number. The highlight card also links to the full /suporte hub, carrying
// the trilha context so that page can pre-fill too.
...
// Link to the full support hub, carrying trilha context as query params so /suporte
// can pre-fill its own WhatsApp/e-mail message for the same student.
export function supportPageUrl(context = {}, config = CONFIG) {
...
// Prominent card for the friction points: login, código, registro, bloqueado, erro.
export function highlightHtml(context = {}, config = CONFIG) {
  return (
    '<div class="cdx-support-highlight">' +
      ...
      '<a class="cdx-support-more" href="' + esc(supportPageUrl(context, config)) + '">' + esc(t('support.more')) + '</a>' +
    '</div>'
  );
}
