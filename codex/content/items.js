...
// Notices route through the single shared module (js/notice.js):
//   notice.ok       success/confirmation (green, transient)
//   notice.error    user-actionable error / validation (red)
//   notice.internal admin/technical error → debug pill; visible only when the
//                   pill is ON (Elder's rule: users see only what they can act on)
...
function _bulkDelete() {
  ...
  if (!ids.length) { notice.error(t('content.no_selection')); return; }
  _openConfirm({
    ...
    onConfirm() {
      ...
      api.bulkDeleteItems({ ids, _silent: true }).then(() => {
        notice.ok(ids.length + ' ' + t('content.items_deleted_suffix'));
  ...
  _openConfirm({
    ...
    onConfirm() {
      ...
      api.deleteItem({ id, _silent: true }).then(() => {
        notice.ok(t('content.item_deleted'));
      }).catch((e) => {
...
function _duplicateItem(id) {
  api.duplicateItem({ id, _silent: true }).then((d) => {
    if (d && d.item) {
      ...
      _renderItems();
      notice.ok(t('content.item_duplicated'));
    }
...
function _openItemEditorFull(item, prefill, aiContext) {
  ...
  itemForm.mount(bd.querySelector('.cdx-modal-body'), {
    ...
    onSave: () => {
      _closeModal(bd);
      notice.ok(isEdit ? t('content.item_updated') : t('content.item_created'));
...
function _openTypeCreateForm(callback) {
  ...
  bd.querySelector('[data-act="ok"]').addEventListener('click', () => {
    ...
    if (!label || !slug) { notice.error(t('content.name_required')); return; }
    api.createType({ slug, label, icon: chosenIcon }).then(() => _loadTypes()).then(() => {
      notice.ok(t('content.type_created'));
      done(slug);
    }).catch((e) => notice.internal(_err(e)));
  });
}
    ...
    if (action === 'glyph') {
      _openGlyphPicker(ty.icon, (iconVal) => {
        api.updateType({ slug, icon: iconVal }).then(() => _loadTypes()).then(() => {
          render(); _renderItems(); notice.ok(t('content.type_updated'));
      ...
      _openConfirm({
        ...
        onConfirm() {
          ...
          api.deleteType({ slug }).then(() => _loadTypes()).then(() => {
            render(); _renderItems(); notice.ok(t('content.type_deleted'));
          }).catch((er) => {
      ...
      _openPrompt({
        ...
        onSubmit(n) {
          ...
          api.renameTag({ id, label: n }).then(() => _loadTags()).then(() => {
            render(); notice.ok(t('content.tag_renamed')); _loadItems({ silent: true });
          }).catch((er) => notice.internal(_err(er)));
        },
      });
    } else if (action === 'delete') {
      _openConfirm({
        ...
        onConfirm() {
          api.deleteTag({ id }).then(() => _loadTags()).then(() => {
            render(); notice.ok(t('content.tag_deleted')); _loadItems({ silent: true });
          }).catch((er) => notice.internal(_err(er)));
        },
      });
    }
