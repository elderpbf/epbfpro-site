...
// ClassVault engine (Codex embed). Display name: PensoCodex. De-booted copy of
// backstage/classvault/js/classvault.js: the self-booting page script became a
// mountable window.AulaEngine = { mount, unmount }. Rendering, callWorker calls,
// cv-/sub/bs- classes and Portuguese strings are kept verbatim (vendored legacy,
// exempt from the cdx-/facade/i18n contract, like the Labs/Drive wrappers).

// Codex embed shim: Aula's turma selector + text-resize live in the backstage
// topbar on the standalone page. Inside Codex there is no backstage topbar, so
// render those controls into a thin control bar at the top of the sidebar head.
if (!window.Topbar) {
  window.Topbar = {
    init: function(){}, codexTabs: function(){ return []; },
    setSubtitle: function(){}, setSelector: function(cfg){ _aulaShimSelector(cfg); },
    addItem: function(cfg){ return _aulaShimAddItem(cfg); }
  };
}

// Ensure (and return) the .cv-aula-controls bar prepended into .cv-sm-head.
// _renderSearchInput() resets .cv-sm-head innerHTML, so each shim call recreates
// the bar if missing, keeping it robust against call ordering.
function _aulaEnsureControls() {
  var head = document.querySelector('.cv-sm-head');
  if (!head) return null;
  var bar = head.querySelector('.cv-aula-controls');
  if (!bar) {
    bar = document.createElement('div');
    bar.className = 'cv-aula-controls';
    head.insertBefore(bar, head.firstChild);
  }
  return bar;
}

// Topbar.setSelector shim: render the turma <select> into the control bar.
function _aulaShimSelector(cfg) {
  var bar = _aulaEnsureControls();
  if (!bar) return;
  var old = bar.querySelector('.cv-aula-turma-select');
  if (old) old.remove();
  var sel = document.createElement('select');
  sel.className = 'cv-aula-turma-select';
  var options = (cfg && cfg.options) || [];
  ...
  if (cfg && cfg.value != null) sel.value = cfg.value;
  sel.addEventListener('change', function() {
    if (cfg && typeof cfg.onChange === 'function') cfg.onChange(sel.value);
  });
  bar.insertBefore(sel, bar.firstChild);
}

// Topbar.addItem shim: append a control button into the control bar and return
// it so the caller can set its .id (matches the legacy Topbar.addItem contract).
function _aulaShimAddItem(cfg) {
  var bar = _aulaEnsureControls();
  if (!bar) return null;
  var btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'cv-aula-ctrl-btn';
  if (cfg && cfg.title) btn.title = cfg.title;
  if (cfg && cfg.icon) btn.innerHTML = cfg.icon;
  if (cfg && typeof cfg.onClick === 'function') btn.addEventListener('click', cfg.onClick);
  bar.appendChild(btn);
  return btn;
}
...
(function() {
  ...
  CVTextResize.apply();
  // Codex embed: the +A/-A buttons were injected into the backstage topbar at
  // load. Defer to mount time (the Topbar shim needs the sidebar to exist).
  // STEP lives in this IIFE scope, so the wiring fn is defined here.
  window._aulaMountTextResizeButtons = function() {
    if (!window.Topbar) return;
    // Pass HTML through `icon` so Topbar.addItem applies the bs-icon-btn box
    // styling (32x32 square) to match the mock's icon-row appearance.
...
// ---- Codex embed: mount / unmount --------------------------------
// _aulaMount injects the cv-shell markup (verbatim from the standalone
// index.html main.bs-main), then runs the former top-level boot / wiring
// calls in the SAME order they ran at page load.
function _aulaMount(rootEl) {
  if (!rootEl) return;
  rootEl.innerHTML =
    '<div class="cv-shell">' +
      '<aside class="cv-sm">' +
        '<div class="cv-sm-head"></div>' +
        '<div class="cv-sm-preset" id="cv-sm-preset"></div>' +
        '<div class="cv-sm-body"></div>' +
        '<div class="cv-sm-pinned"></div>' +
      '</aside>' +
      '<section class="cv-main">' +
        '<div class="cv-main-crumb"></div>' +
        '<div class="cv-main-view"></div>' +
      '</section>' +
    '</div>';

  // Former top-level boot side-effects (page-load order preserved):
...
// _aulaUnmount tears down the only persistent thing the engine creates: the
// _watchBodyForPin MutationObserver. The setTimeouts elsewhere are short-lived;
// the document-level ctx-menu listeners are added/removed by _openContextMenu/
// _closeContextMenu themselves. The wrapper clears rootEl's DOM. Single instance
// per session, so no deeper teardown is needed.
function _aulaUnmount() {
  if (window.ClassVault && ClassVault._aulaPinObserver) {
    try { ClassVault._aulaPinObserver.disconnect(); } catch (e) {}
    ClassVault._aulaPinObserver = null;
  }
}
...
window.AulaEngine = { mount: _aulaMount, unmount: _aulaUnmount };
